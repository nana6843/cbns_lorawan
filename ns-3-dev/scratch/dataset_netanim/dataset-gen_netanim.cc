...
std::string csvPath = "/home/nru/CBNS_NEW2/ns-3-dev/scratch/dataset-gen/experiment_schedule.csv";  // Path to CSV


// Network settings
int nDevices = 500;                 //!< Number of end device nodes to create
...
std::string animFile = "lorawan-animation.xml";  // Output animation file
...
main(int argc, char* argv[])
{
    ...
    std::string positionsCSV = "/home/nru/CBNS_NEW2/ns-3-dev/scratch/dataset-gen//node_positions.csv";
    ...
    // NetAnim Setup //
    ////////////////
    NS_LOG_INFO("Creating animation file...");

        // Create animation interface
    AnimationInterface anim(animFile);
    anim.SetMaxPktsPerTraceFile(500000);

    // ✅ DISABLE PACKET ANIMATION (supaya tidak berantakan)
    anim.EnablePacketMetadata(false);

    std::cout << "\n=== Setting up NetAnim visualization ===" << std::endl;

    // Set position untuk Network Server (pojok kanan atas)
    Ptr<ConstantPositionMobilityModel> nsMobility = CreateObject<ConstantPositionMobilityModel>();
    nsMobility->SetPosition(Vector(radiusMeters + 1000, radiusMeters + 1000, 0.0));
    networkServer->AggregateObject(nsMobility);

    // Network Server - hidden di pojok
    uint32_t nsId = networkServer->GetId();
    anim.UpdateNodeDescription(nsId, "");
    anim.UpdateNodeColor(nsId, 200, 200, 200);  // Gray (less visible)
    anim.UpdateNodeSize(nsId, 2.0, 2.0);

    // ✅ End Devices - BULAT dengan WARNA berdasarkan DR (NO LABELS!)
    std::map<uint8_t, int> drCount;
    int nodesWithPosition = 0;
    ...
    for (uint32_t i = 0; i < endDevices.GetN(); ++i)
    {
        ...
        // Ambil posisi dari fixedPositions
        if (i >= fixedPositions.size()) {
            std::cerr << "⚠️  No position for node " << i << std::endl;
            continue;
        }
        
        Vector originalPos = fixedPositions[i];
        
        // Shift ke koordinat positif untuk NetAnim (center at radiusMeters)
        double netanimX = originalPos.x + radiusMeters;
        double netanimY = originalPos.y + radiusMeters;
        
        // Debug print untuk node pertama
        if (i < 3) {
            std::cout << "Node " << i << ": orig=(" << originalPos.x << "," << originalPos.y 
                    << ") → netanim=(" << netanimX << "," << netanimY << ")" << std::endl;
        }
        
        // Set position
        anim.SetConstantPosition(node, netanimX, netanimY);
        nodesWithPosition++;
        
        // Get DR from config
        uint8_t dr = 7;
        if (experimentConfigs.find(i) != experimentConfigs.end())
        {
            dr = experimentConfigs[i].dr;
        }
        drCount[dr]++;
        
        // ✅ CRITICAL: Empty string untuk NO LABELS
        anim.UpdateNodeDescription(nodeId, "");
        
        // ✅ WARNA berdasarkan Data Rate
        switch(dr) {
            case 0:  // SF12 - Red
                anim.UpdateNodeColor(nodeId, 255, 0, 0);
                break;
            case 1:  // SF11 - Orange
                anim.UpdateNodeColor(nodeId, 255, 140, 0);
                break;
            case 2:  // SF10 - Yellow
                anim.UpdateNodeColor(nodeId, 255, 215, 0);
                break;
            case 3:  // SF9 - Green
                anim.UpdateNodeColor(nodeId, 50, 205, 50);
                break;
            case 4:  // SF8 - Cyan
                anim.UpdateNodeColor(nodeId, 0, 191, 255);
                break;
            case 5:  // SF7 - Blue
                anim.UpdateNodeColor(nodeId, 0, 0, 255);
                break;
            default:  // Unknown - Gray
                anim.UpdateNodeColor(nodeId, 128, 128, 128);
                break;
        }
    ...
    std::cout << "✅ Positioned " << nodesWithPosition << "/" << endDevices.GetN() << " nodes" << std::endl;
    std::cout << "   DR distribution:" << std::endl;
    for (auto const& pair : drCount)
    {
        std::cout << "      DR" << (int)pair.first << ": " << pair.second << " nodes" << std::endl;
    }

    // ✅ Gateway - BULAT BESAR di tengah (Purple)
    for (uint32_t i = 0; i < gateways.GetN(); ++i)
    {
        Ptr<Node> node = gateways.Get(i);
        uint32_t nodeId = node->GetId();
        
        // Gateway di center canvas
        double netanimX = radiusMeters;
        double netanimY = radiusMeters;
        
        anim.SetConstantPosition(node, netanimX, netanimY);
        
        std::cout << "Gateway nodeId=" << nodeId << " at (" << netanimX << "," << netanimY << ")" << std::endl;
        
        // Gateway description (empty atau simple)
        anim.UpdateNodeDescription(nodeId, "GW");
        
        // Purple - bulat besar
        anim.UpdateNodeColor(nodeId, 138, 43, 226);  // BlueViolet
        anim.UpdateNodeSize(nodeId, 20.0, 20.0);  // Very large
    }

    std::cout << "✅ NetAnim setup complete!" << std::endl;
    std::cout << "\n📊 Color Legend:" << std::endl;
    std::cout << "   🔴 Red    = DR0 (SF12) - Slowest, farthest" << std::endl;
    std::cout << "   🟠 Orange = DR1 (SF11)" << std::endl;
    std::cout << "   🟡 Yellow = DR2 (SF10)" << std::endl;
    std::cout << "   🟢 Green  = DR3 (SF9)" << std::endl;
    std::cout << "   🔵 Cyan   = DR4 (SF8)" << std::endl;
    std::cout << "   🔵 Blue   = DR5 (SF7) - Fastest, nearest" << std::endl;
    std::cout << "   🟣 Purple = Gateway\n" << std::endl;
    ...
    for (uint32_t i = 0; i < endDevices.GetN(); ++i)
    {
        ...
        filename_1 << "/home/nru/CBNS_NEW2/ns-3-dev/scratch/dataset-gen/results_experiment_" << experimentId << ".csv";

        std::ofstream csvFile(filename_1.str());
        csvFile << "node_id,x,y,z,distance,dr,freq,txPower,TxPackets,RxPackets,TotalToA_ms,AvgToA_ms\n";
