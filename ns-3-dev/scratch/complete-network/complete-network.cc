...
// void
// PrintRssi(double rssi)
// {
//     std::cout << Simulator::Now().GetSeconds()
//               << "s  RSSI=" << rssi << " dBm" << std::endl;
// }
...
// {
//     std::cout << Simulator::Now().GetSeconds()
//               << "s  SNR=" << snr << " dB" << std::endl;
// }

// // === Global callback functions ===
// void RssiTrace(double rssi)
// {
//     std::cout << Simulator::Now().GetSeconds()
//               << "s  TRACE_RSSI = " << std::fixed << std::setprecision(2)
//               << rssi << " dBm" << std::endl;
// }

// void SnrTrace(double snr)
// {
//     std::cout << Simulator::Now().GetSeconds()
//               << "s  TRACE_SNR  = " << std::fixed << std::setprecision(2)
//               << snr << " dB" << std::endl;
// }
...
main(int argc, char* argv[])
{
    ...
    for (auto j = endDevices.Begin(); j != endDevices.End(); ++j)
    {
        ...
        if (mac)
        {
            ...
            // command ini untuk set tx power dan data rate secara manual
            mac->SetTransmissionPowerDbm(7.0);
            // mac->SetDataRate(4);
            // mac->SetCurrentFrequency(868500000.0); 
            txPower = mac->GetTransmissionPowerDbm();
            dr = mac->GetDataRate();
            freq = mac->GetCurrentFrequency();


            // ↓↓↓ ADD THIS: Get device address ↓↓↓
            ...
            double distance = std::sqrt(std::pow(pos.x - gwPos.x, 2) +std::pow(pos.y - gwPos.y, 2) +std::pow(pos.z - gwPos.z, 2));
            
            // ↓↓↓ ADD THIS: Store node information ↓↓↓
            NodeInfo info;
            info.nodeId = nodeId;
            info.position = pos;
            ...
            addressToNodeInfo[address] = info;  // Store in map
    ...
    for (uint32_t i = 0; i < endDevices.GetN(); ++i)
    {
        ...
        // ============================
        std::ofstream csvFile("lorawan_results.csv");
        csvFile << "node_id,x,y,z,distance,dr,freq,txPower,TxPackets,RxPackets,TotalToA_ms\n";
        ...
        csvFile.close();
        std::cout << "✅ Results saved to lorawan_results.csv" << std::endl;
