#!/usr/bin/env python3
"""
LoRaWAN Network-Aware Config Predictor - LIGHTGBM VERSION
Just 2 functions: predict_best() and process_all_nodes()
Uses LightGBM model for faster inference
"""
...
# ⭐ Load LightGBM model instead of RF
model = joblib.load('lorawan_adr_model_lgbm.pkl')
...
def predict_best(distance, freq_usage={868.1: 0, 868.3: 0, 868.5: 0}, total_nodes=1):
    """
    Predict best config for one node (network-aware) using LightGBM
...
def process_all_nodes(distances):
    """
    Process all nodes with network-aware frequency allocation (LightGBM)
...
def generate_positions(num_nodes, radius_meters):
    ...
    # print(positions)
    nama_file = f'node_positions_lgbm_{num_nodes}_{radius_meters}.csv'
...
def ml_predict(num_nodes, radius_meters):
    print(f"\n🌐 Processing {num_nodes} nodes in {radius_meters}m radius with LightGBM")
    print("="*60)
    params_values = []
    ...
    nama_file = f'experiment_schedule_lgbm_{num_nodes}_{radius_meters}.csv'
    # print(params_values)
    df = pd.DataFrame(params_values)
    df.to_csv(nama_file, index=False)


    return print(f"✓ Saved Position to: node_positions_{num_nodes}_{radius_meters}.csv & parameters to: {nama_file}")

# ========== USAGE ==========
if __name__ == "__main__":
    print("="*70)
    print("LoRaWAN Config Predictor - LightGBM Version")
    print("Faster inference, better accuracy")
    print("="*70)
