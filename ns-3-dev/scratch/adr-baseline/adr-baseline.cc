...
std::map<uint32_t, NodeExperimentConfig> LoadExperimentConfig(uint32_t experimentId, std::string csvPath)
{
    ...
    if (!file.is_open())
    {
        std::cout << "⚠️ Warning: CSV " << csvPath << " not found. "
                << "Using default device configuration (ADR controls DR/TxPower).\n";

        return configs;   // return empty map, no crash
    }
...
std::string csvPath = "/home/nru/CBNS_NEW2/ns-3-dev/scratch/adr-baseline/experiment_schedule_20_5000.csv";  // Path to CSV
...
void AdrDrTrace(uint8_t oldDr, uint8_t newDr)
{
    std::cout << "ADR: DR " << int(oldDr) << " → " << int(newDr)
              << " @ " << Simulator::Now().GetSeconds() << "s\n";
}

void AdrTxPowerTrace(double oldTx, double newTx)
{
    std::cout << "ADR: TxPower " << oldTx << " dBm → " << newTx
              << " dBm @ " << Simulator::Now().GetSeconds() << "s\n";
}
...
main(int argc, char* argv[])
{
    ...
    // make ADR more eager for testing:
    ...
    std::string positionsCSV = "/home/nru/CBNS_NEW2/ns-3-dev/scratch/adr-baseline/node_positions_20_5000.csv";
    ...
    nsHelper.SetAdr("ns3::AdrComponent");  // default ADR algorithm

    // === Activate ADR on Network Server ===
    ...
    ************************/
    ...
    for (uint32_t i = 0; i < endDevices.GetN(); ++i)
    {
        ...
        filename_1 << "/home/nru/CBNS_NEW2/ns-3-dev/scratch/adr-baseline/results_experiment_20_5000_" << experimentId << ".csv";
